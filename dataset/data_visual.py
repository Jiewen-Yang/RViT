...
class visual_data(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        item, frame_list = self.process_frame(key_id-1)
        return item, self.label[str(key_id)], frame_list
    ...
    def process_frame(self, index):
        ...
        if i < self.sample_length-1:
            frames[i:] = img
            release = i-1
        return self.transform(frames), frame_list
